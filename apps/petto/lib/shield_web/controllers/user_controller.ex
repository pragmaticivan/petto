defmodule ShieldWeb.UserController do
  use ShieldWeb, :controller
  ...
  @views Application.get_env(:petto, :views)
  ...
  plug(
    Authable.Plug.Authenticate,
    [scopes: ~w(session read write)] when action in [:change_password]
  )

  plug(
    Authable.Plug.UnauthorizedOnly
    when action in [:register, :login, :confirm, :recover_password, :reset_password]
  )

  plug(
    Shield.Arm.Confirmable,
  ...
  def confirm(conn, _) do
    @renderer.render(conn, :unprocessable_entity, %{
      errors: %{details: "Missing confirmation_token data!"}
    })
  end
  ...
  def recover_password(conn, _) do
    @renderer.render(conn, :unprocessable_entity, %{errors: %{details: "Missing email data!"}})
  end
  ...
  def register(conn, _) do
    @renderer.render(conn, :unprocessable_entity, %{
      errors: %{details: "Missing email or password data!"}
    })
  end

  # POST /users/login
  def login(conn, %{"user" => %{"email" => email, "password" => password} = user_params} = params)
      when is_binary(password) and is_binary(email) do
  ...
  def reset_password(conn, _) do
    @renderer.render(conn, :unprocessable_entity, %{
      errors: %{details: "Missing password or reset_token data!"}
    })
  end
  ...
  def change_password(conn, _) do
    @renderer.render(conn, :unprocessable_entity, %{
      errors: %{details: "Missing password or old_password data!"}
    })
  end
end
