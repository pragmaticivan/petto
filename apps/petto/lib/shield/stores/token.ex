defmodule Shield.Store.Token do
  ...
  defp find_client({:error, params}), do: {:error, params}

  defp validate_client_token_ownership(
         {:ok, %{"token" => token, "client_id" => client_id} = params}
       ) do
  ...
  defp validate_client_token_ownership({:error, params}), do: {:error, params}
end
