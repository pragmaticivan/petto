defmodule Shield.Policy.User.ResetPassword do
  ...
  defp delete_reset_token({:error, {_status, _errors} = opts}), do: {:error, opts}

  defp change_password({:ok, %{"token" => token, "password" => password} = params}) do
    changeset = @resource_owner.password_changeset(token.user, %{password: password})
  ...
  defp change_password({:error, {_status, _errors} = opts}), do: {:error, opts}
end
