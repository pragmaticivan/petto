defmodule Shield.Policy.User.Login do
  ...
  defp validate_email_password_match({:error, {_status, _errors} = opts}), do: {:error, opts}

  defp validate_email_confirmation({:ok, %{"user" => user} = params}) do
    is_email_confirmation_required = Application.get_env(:petto, :confirmable)
    is_email_confirmed = Map.get(user.settings || %{}, "confirmed", false)

    case {is_email_confirmation_required, is_email_confirmed} do
      {false, _} -> {:ok, params}
      {true, true} -> {:ok, params}
      {_, _} -> {:error, {:unauthorized, %{email: ["Email confirmation required."]}}}
    end
  end

  defp validate_email_confirmation({:error, {_status, _errors} = opts}), do: {:error, opts}

  defp validate_one_time_password({:ok, %{"user" => user} = params}) do
    is_otp_check_required = Application.get_env(:petto, :otp_check)
  ...
  defp validate_one_time_password({:error, {_status, _errors} = opts}), do: {:error, opts}
    ...
    case OneTimePasswordArm.find_otp_secret_token(user) do
      ...
      token ->
        ...
        case OneTimePasswordArm.is_valid?(token.value, otp_value) do
          false -> {:error, {:unauthorized, %{otp_value: ["Invalid one time password"]}}}
          true -> {:ok, params}
        end
    end
  end
  ...
  defp insert_session_token({:error, {_status, _errors} = opts}), do: {:error, opts}
end
