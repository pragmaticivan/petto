defmodule ShieldWeb.SettingControllerTest do
  ...
  test "enable otp when otp enabled by config", %{conn: conn, otp_secret: otp_secret, otp_value: otp_value} do
    Application.put_env(:petto, :otp_check, true, persistent: true)
  ...
  test "enable otp when otp is disabled by config", %{conn: conn, otp_secret: otp_secret, otp_value: otp_value} do
    Application.put_env(:petto, :otp_check, false, persistent: true)
    data = %{"setting" => %{"one_time_password" => %{"action" => "enable",
      "otp_secret" => otp_secret, "otp_value" => otp_value}}}
    conn = put(conn, setting_path(conn, :update, "one_time_password", data))
