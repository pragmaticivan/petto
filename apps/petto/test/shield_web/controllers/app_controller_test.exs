defmodule ShieldWeb.ClientControllerTest.AppControllerTest do
  ...
  test "lists all entries on index for current user", %{conn: conn} do
    ...
    assert Enum.count(json_response(conn, 200)["apps"]) == 1
  end
  ...
  test "does not show resource and instead throw error when id not belongs to current user", %{
    conn: conn,
    app2: app2
  } do
  ...
  test "does not authorize app for current user when client not found", %{
    conn: conn,
    client: client
  } do
    params = %{
  ...
  test "does not delete chosen resource that not belongs to current user", %{
    conn: conn,
    app2: app2
  } do
