defmodule ShieldWeb.UserControllerTest do
  ...
  setup %{conn: conn} do
    Application.put_env(:petto, :confirmable, false)
    Application.put_env(:petto, :otp_check, false)
  ...
  test "a GET request to /confirm with a valid token", %{conn: conn, user: user} do
    Application.put_env(:petto, :confirmable, true)
  ...
  test "a POST request to /login with valid email & password when one time password enabled", %{conn: conn, user: user} do
    Application.put_env(:petto, :otp_check, true)
